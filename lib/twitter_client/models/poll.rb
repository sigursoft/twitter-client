  ...
  class Poll
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `TwitterClient::Poll` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `TwitterClient::Poll`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def valid?
      return false if !@duration_minutes.nil? && @duration_minutes > 10080
      return false if !@duration_minutes.nil? && @duration_minutes < 5
      return false if @id.nil?
      return false if @id !~ Regexp.new(/^[0-9]{1,19}$/)
      return false if @options.nil?
      return false if @options.length > 4
      return false if @options.length < 2
      voting_status_validator = EnumAttributeValidator.new('String', ["open", "closed"])
      return false unless voting_status_validator.valid?(@voting_status)
      true
    end
    ...
    # @param [Object] voting_status Object to be assigned
    def voting_status=(voting_status)
      validator = EnumAttributeValidator.new('String', ["open", "closed"])
      unless validator.valid?(voting_status)
        fail ArgumentError, "invalid value for \"voting_status\", must be one of #{validator.allowable_values}."
      end
      @voting_status = voting_status
    end
    ...
    def hash
      [duration_minutes, end_datetime, id, options, voting_status].hash
    end
